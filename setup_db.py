...
cur.execute(('CREATE TABLE IF NOT EXISTS Bantags'
             '(Id INTEGER PRIMARY KEY, tag TEXT)'))
cur.execute(('CREATE TABLE IF NOT EXISTS Banusers'
...
for each in licenses[0]:
    ...
    if licid == 2:
        name = 'CC BY-NC'
        cur.execute('INSERT INTO Licenses(lic_id, name, url) VALUES(?,?,?)',
                    (licid, name, each.get('url')))
    elif licid == 4:
        name = 'CC BY'
        cur.execute('INSERT INTO Licenses(lic_id, name, url) VALUES(?,?,?)',
                    (licid, name, each.get('url')))
